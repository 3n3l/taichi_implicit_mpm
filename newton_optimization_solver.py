...
class NewtonSolver:
    ...
    def initialize(self, dim, shape, functions_dict, dtype=ti.f32):
        ...
        self.linear_solver.initialize(
            dim=dim, shape=shape, functions_dict={"multiply": self.multiply, "project": self.project}
        )
    ...
    @ti.kernel
    def compute_residual_norm(self, x: ti.template()) -> ti.f32:  # pyright: ignore
        result = 0.0
    ...
    def linear_solve(self, x, b, precondtioner):
        self.linear_solver.solve(x, b, precondtioner)  # pyright: ignore

    @ti.kernel
    def update_step(self, dst: ti.template(), src: ti.template(), scale: ti.f64):  # pyright: ignore
        for I in ti.grouped(src):
            dst[I] += scale * src[I]

    @ti.kernel
    def update_general(
        self, dst: ti.template(), src1: ti.template(), src2: ti.template(), s: ti.f64
    ):  # pyright: ignore
        for I in ti.grouped(dst):
            dst[I] = src1[I] + s * src2[I]

    @ti.kernel
    def copy(self, dst: ti.template(), src: ti.template()):  # pyright: ignore
        for I in ti.grouped(dst):
            dst[I] = src[I]
    ...
    def clear_step_direction(self):
        for I in ti.grouped(self.step_direction):
            for d in ti.static(range(self.dim)):  # pyright: ignore
                self.step_direction[I][d] = 0.0  # pyright: ignore
    ...
    def solve(self, x, preconditioner):
        ...
        if self.line_search:
            ...
            E_0 = self.total_energy()  # pyright: ignore
        ...
        for n in range(self.max_iterations):
            ...
            if self.line_search:
                ...
                for l in range(self.line_search_step):
                    ...
                    self.update_simulation_state(x)
                    E = self.total_energy()  # pyright: ignore
